...
% Version:  1.0
% Date:     March 1 2020
% Purpose:  Segmentation approaches for detecting background noise in concrete images without cracks
clear all;
clc;
close all;
metrics = [];
...
for i = 1 : length(folderInfo)
    ...
    totalPixelCount = length(orig) * length(orig);
    kMeansPixelCount = totalPixelCount-nnz(kMeansRemoveClutterImg);
    otsuPixelCount = totalPixelCount-nnz(removeClutterImgOtsu);
    otsuError = (abs(otsuPixelCount-totalPixelCount)/totalPixelCount) * 100;
    kMeansError = (abs(kMeansPixelCount-totalPixelCount)/totalPixelCount) * 100;
    metrics=[metrics ;i+5 totalPixelCount otsuPixelCount kMeansPixelCount otsuError kMeansError];
...
matrix2latex(metrics, 'c:\img\noncrack\metrics.tex',  'alignment', 'l', 'format', '%-1.0f', 'size', 'tiny');
